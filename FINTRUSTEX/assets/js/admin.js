...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateUserList = async () => {
    try {
      ...
      if (userList) {
        userList.querySelector("tbody").innerHTML = data.users
          .map(
            (user) => `
          <tr>
            <td>${user.id}</td>
  ...
  const updateTransactionList = async () => {
    try {
      ...
      if (transactionList) {
        transactionList.querySelector("tbody").innerHTML = data.transactions
          .map(
            (tx) => `
          <tr>
            <td>${tx.id}</td>
  ...
  const initAdminActions = () => {
    document.addEventListener("click", (e) => {
      if (e.target.classList.contains("suspend-user")) {
        ...
        showToast(
          `User ${userId} ${e.target.textContent.includes("Suspend") ? "suspended" : "activated"}`,
          "success",
        );
  ...
  try {
    ...
    if (activityList) {
      activityList.innerHTML = activities
        .map(
          (activity) => `
          <li>${activity.description} at ${new Date(activity.timestamp).toLocaleString()}</li>
        `,
        )
        .join("");
    }
...
document
  .getElementById("export-users-csv")
  .addEventListener("click", async () => {
    const data = await fetchAdminData();
    const userData = data.users.map((u) => [u.id, u.username, u.email]);
...
document
  ...
  .addEventListener("click", async () => {
    ...
    exportToCSV(transactionData, "transactions.csv");
  });
